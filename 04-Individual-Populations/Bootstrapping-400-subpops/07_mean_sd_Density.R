...
for (i in 1:100) {
  ...
  for (pop in 1:4) {
    ...
    temp_data <- temp_data[, c("gene", "density")]
    names(temp_data) <- c("gene", "density")
...
for (name in names(data_list)) {
  ...
  # Rename the 'density' column to make it unique across data frames
  temp_data <- rename(temp_data, !!paste0("density_", name) := density)
...
# Add a column with the mean density value for each gene
final_data <- final_data %>%
  rowwise() %>%
  mutate(
    mean_density = mean(c_across(starts_with("density_")), na.rm = TRUE),
    sd_density = sd(c_across(starts_with("density_")), na.rm = TRUE)
  ) %>%
  ungroup()
...
write.table(bias_and_all, "results/Master_density_combined_400.txt",
            col.names = TRUE,
            quote = FALSE)
