...
for (pop in populations) {
  ...
  filepath <- paste0('set-9/set-9-', pop, '-Final_Genes_Scored.txt')
...
write.table(bias_and_all,
            paste0("set-9/set-9-", pop, "-ranked-by_dens.txt"),
            quote = FALSE,
            col.names = TRUE,
            row.names = FALSE)

write.table(bias_and_all$gene,
            paste0("set-9/set-9-", pop, "-All-Genes.txt"),
            quote = FALSE,
            col.names = FALSE,
            row.names = FALSE)
...
write.table(candidates$gene,
            paste0("set-9/set-9-", pop, "-Candidate_Genes_only.txt"),
            quote = FALSE,
            col.names = FALSE,
            row.names = FALSE)
